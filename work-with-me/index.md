---
layout: page
practice: true
title: Work with me
permalink: /work-with-me/
description: >-
  Product and engineering assessment and hands-on leadership for founders of growing
  product companies. Decide what needs to change, then put it into practice.
---

<p class="lede">I help founders turn growth plans into product direction, the right team, and working software - and lead the changes alongside them.</p>

Choose the situation closest to yours. We’ll agree the scope around the decision or change you need to make.

{% include offer_chooser.html %}

<p>{% include conversation_cta.html %}</p>

## Start with a clear plan

An independent **product and engineering assessment** examines your priorities, working evidence, team and technical context. I gather input through a short questionnaire, selected individual conversations and existing work, with the approach agreed around your team’s size and the decision.

You receive a concise decision document: what is limiting progress, what your team needs, and a prioritised plan with proposed owners and success checks. We work through it together.

[Explore the assessment &rarr;]({{ '/assessment/' | relative_url }})

## Review a hiring plan

You’re preparing to open engineering roles and want an independent view of which hires are justified, what they should own and which should come first.

The **Hiring-plan second opinion** reviews one proposed hiring plan against your product goals, team shape and available delivery evidence. You receive a short decision memo with recommendations, assumptions to test and questions that could change the conclusion.

This is a standalone, fixed-fee review. We agree scope, inputs and schedule before starting.

[Explore the Hiring-plan second opinion &rarr;]({{ "/hiring-plan-second-opinion/" | relative_url }})

## Lead the changes

I take responsibility for agreed work alongside you and your team: product priorities, people management and hiring, architecture and hands-on implementation, or improving how discovery and delivery connect. We select the priorities your business needs.

This engagement focuses on a specific change with a defined outcome and a bounded piece of work. You see the decisions, what has shipped or changed, and what the evidence says. We review progress against that outcome and agree any further work separately.

<details class="engagement-details" markdown="1">
<summary>How we agree scope, responsibility and fees</summary>

Before starting, we agree the changes I will lead, the decisions I can make and those that remain with you or existing leaders. We set my involvement, your team's responsibilities and the cadence for reviewing progress.

Implementation has its own scope and monthly fee, separate from the assessment. Duration and review points are agreed for your situation. As the changes take hold, we review what support remains useful and transfer responsibility to your team where appropriate.

If you already have a clear plan, an assessment is not a prerequisite.

</details>

## From prototype to production

You’ve validated an idea with a working prototype, perhaps built with AI. I work hands-on to strengthen the application, decide what to build next and establish the smallest team needed to take ownership.

We review product and customer evidence first, then agree scope and fee. You can start without an existing engineering team.

[Explore From prototype to production &rarr;]({{ '/prototype-to-production/' | relative_url }})

## Fractional CTO & CPTO engagement

Your company needs ongoing product and technology leadership, with someone taking responsibility for agreed decisions, teams and delivery.

I work alongside you and your existing leaders, connecting product direction, team leadership and hands-on engineering. The engagement covers an ongoing leadership responsibility as priorities evolve. We agree the scope, decision authority, involvement and monthly fee before starting.

[Explore fractional CTO & CPTO engagement &rarr;]({{ '/fractional-cto-cpto/' | relative_url }})

{% include practice_process.html %}

{% include recommendations.html %}

## Who I work with

Founders of growing product companies, from a validated prototype to organisations of roughly 50 people across product and engineering.

You’re open to testing your assumptions and giving me access to the people and work needed to understand the situation. Existing product and engineering leaders are part of the engagement.

My path at Smily has spanned business and product decisions, people management, organisational change, hands-on engineering and acquisition diligence.

[Read about my experience &rarr;]({{ '/about/' | relative_url }})

## Tell me what needs to change

You don’t need to choose an offering first.

<p>{% include conversation_cta.html %}</p>
